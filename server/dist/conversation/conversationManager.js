 ...
 */
export async function processUserTurn(sessionId, userTranscript) {
 ...
 */
function extractInfo(sessionId, text) {
    const session = getSession(sessionId);
    if (!session)
        return;
    ...
    if (!session.severity) {
        ...
        if (!updates.severity &&
            /\b(mild|moderate|severe|intense|unbearable|slight|terrible|awful|bad|worse|worst)\b/i.test(lower)) {
            const match = lower.match(/\b(mild|moderate|severe|intense|unbearable|slight|terrible|awful|bad|worse|worst)\b/i);
            if (match)
                updates.severity = match[1];
        }
    }
