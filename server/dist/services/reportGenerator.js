 ...
 */
function mergeWithState(llmReport, state) {
    const userMessages = state.messages.filter((m) => m.role === "user").length;
    const report = {
        patientName: llmReport.patientName || state.patientName || "Not provided",
        mainConcern: llmReport.mainConcern || state.mainConcern || "Not provided",
        symptoms: llmReport.symptoms && llmReport.symptoms.length > 0
            ? llmReport.symptoms
            : state.relatedSymptoms || [],
        duration: llmReport.duration || state.duration || "Not provided",
        severity: llmReport.severity || state.severity || "Not provided",
        followUp: llmReport.followUp ||
    ...
    if (report.mainConcern !== "Not provided" &&
...
function buildReportFromState(state) {
    ...
    return {
        ...
        followUp: "Consider consulting a healthcare professional if symptoms persist",
        status: userMessages >= 3 ? "complete" : "incomplete",
